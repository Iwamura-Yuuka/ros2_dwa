...
// コンストラクタ
DWAPlanner::DWAPlanner() : Node("ros2_dwa")
{
    // パラメータの取得
    is_visible_ = this->declare_parameter<bool>("is_visible", true);
    max_vel1_ = this->declare_parameter<double>("max_vel1", 0.35);
    max_vel2_ = this->declare_parameter<double>("max_vel2", 0.25);
    avoid_thres_vel_ = this->declare_parameter<double>("avoid_thres_vel", 0.25);
    min_vel_ = this->declare_parameter<double>("min_vel", 0.0);
    max_yawrate1_ = this->declare_parameter<double>("max_yawrate1", 1.0);
    max_yawrate2_ = this->declare_parameter<double>("max_yawrate2", 0.55);
    turn_thres_yawrate_ = this->declare_parameter<double>("turn_thres_yawrate", 0.25);
    mode_log_time_ = this->declare_parameter<double>("mode_log_time", 5.0);
    max_accel_ = this->declare_parameter<double>("max_accel", 1.0);
    max_dyawrate_ = this->declare_parameter<double>("max_dyawrate", 1.0);
    vel_reso_ = this->declare_parameter<double>("vel_reso", 0.05);
    yawrate_reso_ = this->declare_parameter<double>("yawrate_reso", 0.02);
    dt_ = this->declare_parameter<double>("dt", 0.1);
    predict_time1_ = this->declare_parameter<double>("predict_time1", 2.0);
    predict_time2_ = this->declare_parameter<double>("predict_time2", 5.0);
    roomba_radius_ = this->declare_parameter<double>("roomba_radius", 0.25);
    radius_margin1_ = this->declare_parameter<double>("radius_margin1", 0.1);
    radius_margin2_ = this->declare_parameter<double>("radius_margin2", 0.05);
    goal_tolerance_ = this->declare_parameter<double>("goal_tolerance", 0.5);
    search_range_ = this->declare_parameter<double>("search_range", 0.95);
    weight_heading1_ = this->declare_parameter<double>("weight_heading1", 0.85);
    weight_heading2_ = this->declare_parameter<double>("weight_heading2", 0.7);
    weight_dist1_ = this->declare_parameter<double>("weight_dist1", 1.5);
    weight_dist2_ = this->declare_parameter<double>("weight_dist2", 0.8);
    weight_vel_ = this->declare_parameter<double>("weight_vel", 0.6);
    ...
    sub_obs_poses_  = this->create_subscription<geometry_msgs::msg::PoseArray>("/obstacle_pose", rclcpp::QoS(1).reliable(), std::bind(&DWAPlanner::obs_poses_callback, this, std::placeholders::_1));
    ...
    // timer(10Hz)
    timer_ = this->create_wall_timer(100ms, std::bind(&DWAPlanner::timer_callback, this));
...
void DWAPlanner::local_goal_callback(const geometry_msgs::msg::PointStamped::SharedPtr msg)
{
    ...
    tf2::doTransform(*msg, local_goal_.value(), transform);
}
...
{
    if(not (local_goal_.has_value() and obs_poses_.has_value()))  // msg受信済みか
        return false;

    geometry_msgs::msg::PointStamped local_goal = local_goal_.value();

    const double dx = local_goal.point.x;
    const double dy = local_goal.point.y;
...
void DWAPlanner::change_mode()
{
    ...
    if(mode_log_.size() > hz*mode_log_time_)
...
double DWAPlanner::calc_heading_eval(const std::vector<State>& traj)
{
    ...
    // 最終時刻の位置に対するゴールの方位
    geometry_msgs::msg::PointStamped local_goal = local_goal_.value();
    const double goal_theta = atan2(local_goal.point.y - traj.back().y, local_goal.point.x - traj.back().x);
...
double DWAPlanner::calc_dist_eval(const std::vector<State>& traj)
{
    ...
    {
        geometry_msgs::msg::PoseArray obs_poses = obs_poses_.value();
        for(const auto& obs_pose : obs_poses.poses)
        {
